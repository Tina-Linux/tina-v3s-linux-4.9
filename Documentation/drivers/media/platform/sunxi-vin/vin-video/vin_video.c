...
static void __vin_buf_size(struct vin_core *vinc)
{
	struct vin_vid_cap *cap = &vinc->vid_cap;
	unsigned int size;

	cap->frame.bytesperline[0] = cap->frame.o_width
					* cap->frame.fmt.depth[0] / 8;
	cap->frame.bytesperline[1] = cap->frame.o_width
					* cap->frame.fmt.depth[1] / 8;
	cap->frame.bytesperline[2] = cap->frame.o_width
					* cap->frame.fmt.depth[2] / 8;

	size = cap->frame.o_width * cap->frame.o_height;
	if (cap->frame.fmt.fourcc == V4L2_PIX_FMT_FBC) {
		cap->frame.payload[0] = (CEIL_EXP(cap->frame.o_width, 7)
					* CEIL_EXP(cap->frame.o_height, 5)
					+ CEIL_EXP(cap->frame.o_width, 4)
					* CEIL_EXP(cap->frame.o_height, 2))
						* 96;
	} else {
		cap->frame.payload[0] = size * cap->frame.fmt.depth[0] / 8;
	}
...
static int __vin_set_fmt(struct vin_core *vinc, struct v4l2_format *f)
{
	...
	__vin_buf_size(vinc);
	f->fmt.pix_mp.plane_fmt[0].sizeimage = cap->frame.payload[0];
	f->fmt.pix_mp.plane_fmt[1].sizeimage = cap->frame.payload[1];
	f->fmt.pix_mp.plane_fmt[2].sizeimage = cap->frame.payload[2];
