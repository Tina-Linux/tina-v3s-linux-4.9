...
#define PROC_WAKE_CACHE 16
static inline int xradio_bh_put(struct xradio_common *hw_priv,
		struct sk_buff **skb_p, u8 flags, u8 next_rx)
{
	struct bh_items *item = NULL;
	bh_printk(XRADIO_DBG_TRC, "%s\n", __func__);
	...
	if (likely(item)) {
		...
		if (atomic_add_return(1, &hw_priv->proc.rx_queued) == PROC_WAKE_CACHE ||
			next_rx == 0) {
			wake_up(&hw_priv->proc.proc_wq);
...
#if BH_PROC_RX
			if (SYS_WARN(xradio_bh_put(hw_priv, &skb_rx, flags, ctrl_reg & HIF_CTRL_NEXT_LEN_MASK))) {
