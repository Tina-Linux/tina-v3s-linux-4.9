...
/*----------------------------------------------------------------------------*/
static DEVICE_ATTR(enable,        0660,  mir3da_enable_show,             mir3da_enable_store);
static DEVICE_ATTR(delay,      0660,  mir3da_delay_show,              mir3da_delay_store);
static DEVICE_ATTR(axis_data,    0660,    mir3da_axis_data_show,          NULL);
static DEVICE_ATTR(reg_data,    0660,  mir3da_reg_data_show,           mir3da_reg_data_store);
static DEVICE_ATTR(log_level,  0660,  mir3da_log_level_show,          mir3da_log_level_store);
#ifdef MIR3DA_OFFSET_TEMP_SOLUTION
static DEVICE_ATTR(offset,          0660,  mir3da_offset_show,             mir3da_offset_store);
static DEVICE_ATTR(calibrate_miraGSensor,   0660,  mir3da_calibrate_show,          mir3da_calibrate_store);
#endif
#ifdef FILTER_AVERAGE_ENHANCE
static DEVICE_ATTR(average_enhance, 0660,  mir3da_average_enhance_show,    mir3da_average_enhance_store);
#endif 
// aad cz
static DEVICE_ATTR(int2_enable,  0660,  mir3da_int2_enable_show,             mir3da_int2_enable_store);
static DEVICE_ATTR(int2_clear,    0660, mir3da_int2_clear_enable_show  ,             mir3da_int2_clear_enable_store);
static DEVICE_ATTR(int2_start_status,  0660, mir3da_int2_start_statu_show  ,             mir3da_int2_start_statu_store);

static DEVICE_ATTR(primary_offset,  0660,            mir3da_primary_offset_show,            NULL);
static DEVICE_ATTR(version,         0660,            mir3da_version_show,            NULL);
static DEVICE_ATTR(vendor,          0660,            mir3da_vendor_show,             NULL);
static DEVICE_ATTR(slope_th, 0660,mir3da_slope_th_show, mir3da_slope_th_store);
...
static int gsensor_fetch_sysconfig_para(void)
{
    ...
    if (of_property_read_u32(np, "gsensor_used", &val) < 0)
    {
        MI_ERR("%s: get gsensor_used fail val = %d\n",__func__,val); 
        goto para_get_err;
	}
    device_used = val;
	...
	if (1 == device_used) 
     {
         if (of_property_read_u32(np, "gsensor_twi_id", &val) < 0)
            {
                MI_ERR("%s: get gsensor_twi_id fail\n",__func__); 
                goto para_get_err;
        	}
        twi_id = val;
		if (of_property_read_u32(np, "gsensor_twi_addr", &val) < 0)
            {
                MI_ERR("%s: get gsensor_twi_addr fail\n",__func__); 
                goto para_get_err;
        	}

		if ((val == 0x27) || (val == 0x26))
			normal_i2c[0] = val ;
	    printk( "%s: twi_id is %d  addr = 0x%x \n", __func__, twi_id,normal_i2c[0]);
		ret = 0;
...
module_i2c_driver(mir3da_driver);
