...
#include "../isp_cfg/SENSOR_H/gc2023_mipi_2lane_default_v3_30fps.h"
#include "../isp_cfg/SENSOR_H/gc2023_mipi_2lane_default_v3_60fps.h"


MODULE_AUTHOR("zhouhongbiao");
...
{
	unsigned short rdval1, rdval2, rdval3, rdval4;
...
//      vfe_set_mclk(sd,OFF);
    ...
    case CSI_SUBDEV_PWR_OFF:
      vfe_dev_dbg("CSI_SUBDEV_PWR_OFF! do nothing\n");
	  break;
      //make sure that no device can access i2c bus during sensor initial or power down
      //when using i2c_lock_adpater function, the following codes must not access i2c bus before calling i2c_unlock_adapter
      cci_lock(sd);
...
{
    printk("habo--> gc2023 fse->index=%d \n",fse->index);
    ...
    printk("habo--> gc2023 sensor_win_sizes[fse->index].width=%d \n",sensor_win_sizes[fse->index].width);
    printk("habo--> gc2023 sensor_win_sizes[fse->index].height=%d \n",sensor_win_sizes[fse->index].height);
    return 0;
}
