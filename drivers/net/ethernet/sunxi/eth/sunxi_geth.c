...
#include <linux/pinctrl/consumer.h>
//#include "../../../../../arch/arm/mach-sunxi/include/mach/sun8i/platform-sun8iw8p1.h"
...
static int geth_script_parse(struct platform_device *pdev)
{
...
#endif
    if (!node) {
		printk("can not get dt node for this device.\n");
		return ret;
	}
    #if 0
    ret = of_property_read_u32(node, "gmac0_used", &item);
	if (ret < 0) 
    {
		printk("[phy] gmac0_used type err!\n");
        return ret;
	}
    #else
    item = 2;
	...
	for (cnt = 0; cnt < ARRAY_SIZE(power_tb); cnt++) {
		char *vol;
		size_t len;
		snprintf(power, 15, "gmac0_power%u", (cnt+1));
        #if 0
        ret = of_property_read_string(node, power, &gmac_power);
		if (ret) 
         {
			printk("gmac-power%d: NULL\n", cnt);
			continue;
		}
        len = strlen((char *)gmac_power);
        power_tb[cnt].name = kstrndup((char *)gmac_power, len, GFP_KERNEL);
        #endif
	}
#endif
    ...
    #if 0
    ret = of_property_read_string(node, "gmac0_mode",&gmac_mode);
    if (ret < 0) 
    {
		printk("[phy] gmac_mode type err!\n");
	}
	else
    {
		if (!strncasecmp((char *)gmac_mode, "MII", 3))
			priv->phy_interface = PHY_INTERFACE_MODE_MII;
		else if(!strncasecmp((char *)gmac_mode, "GMII", 4))
			priv->phy_interface = PHY_INTERFACE_MODE_GMII;
		else if(!strncasecmp((char *)gmac_mode, "RMII", 4))
...
{
    printk("habo---> geth_prebe  00000000000000 \n");
	int ret = 0;
	int irq = 0;
	struct resource *res;
    ...
    printk("habo---> geth_prebe  1111111111 \n");
    ...
    printk("habo---> geth_prebe  222222222222 \n");
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "geth_io");
	if (!res) {
		ret =  -ENODEV;
		goto res_err;
	}
    printk("habo---> geth_prebe  333333333333 \n");
    ...
    printk("habo---> geth_prebe  4444444444444 \n");
    ...
    printk("habo---> geth_prebe  5555555555555 \n");
#endif
