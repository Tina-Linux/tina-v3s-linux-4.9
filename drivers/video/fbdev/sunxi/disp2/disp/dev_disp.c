				     ...
				     struct disp_ion_mem *mem)
{
	...
	mem->p_item = disp_dma_map(ion_share_dma_buf_fd(client, mem->handle));
	if (!mem->p_item)
		goto err_phys;
#else
	mem->p_item->fd = ion_share_dma_buf_fd(client, mem->handle);
...
struct disp_ion_mem *disp_ion_malloc(u32 num_bytes, void *phys_addr)
{
	...
	if (NULL == ion_node) {
		__wrn("fail to alloc ion node, size=%lu\n",
		      sizeof(struct disp_ion_list_node));
		return NULL;
	}
