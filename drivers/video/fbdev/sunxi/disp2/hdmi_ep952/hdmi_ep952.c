...
#include "EP952api.h"
//#define EP952_DETECT_HPD //这种方式下hdmi正常使用
#define HDMI_POWER_GPIO_ENABLE
#ifdef HDMI_POWER_GPIO_ENABLE
static struct disp_gpio_set_t powen_gpio[1];
...
static int hdmi_parse_config(void)
{
	...
	char io_name[32];
	  for(i=0; i<28; i++) {
        ...
        ret = disp_sys_script_get_item(key_name, io_name, (int *)gpio_info, 3);
        if(ret == 3) {
            hdmi_io_used[i]= 1;
        }
    }
...
int hdmi_gpio_config(int gpio_id, int bon)
{
	...
	if(gpio_id <= sizeof(hdmi_gpio_used) && hdmi_gpio_used[gpio_id]) {
        ...
        memcpy(gpio_info, &(hdmi_gpio[gpio_id]), sizeof(struct disp_gpio_set_t));
        if(!bon) {
            gpio_info->mul_sel = 7;
        }
...
static int hdmi_pin_config(u32 bon)
{
    ...
    for(i=0; i<28; i++) {
        if(hdmi_io_used[i]) {
            ...
            memcpy(gpio_info, &(hdmi_io[i]), sizeof(struct disp_gpio_set_t));
            if(!bon) {
                gpio_info->mul_sel = 7;
            }
            hdl = disp_sys_gpio_request(gpio_info, 1);
...
static s32 ep952_open(void)
{
	...
	if (!g_ep952_enabled) {
		...
		if (hdmi_source_ops.tcon_enable){
			printk("zhb---> test tcon_enable %s \n",__func__);
			hdmi_source_ops.tcon_enable(ep952_device);
			}
		msleep(100);
#ifndef EP952_DETECT_HPD
    //	EP_HDMI_Init();
#endif
...
static s32 ep952_close(void)
{
	...
	if (g_ep952_enabled) {
		...
		hdmi_pin_config(0);
	#if 0
	#ifndef EP952_DETECT_HPD //ifdef the EP952_DETECT_HPD need to hold power on for ep952 iic 
		printk("zhb---> test no def  EP952_DETECT_HPD \n");
			  ...
			  const struct i2c_device_id *id)
{
	...
	printk("[DISP_I2C] hdmi_i2c_probe\n");
...
static int __init ep952_module_init(void)
{
	int ret, value, i;
	char sub_name[16];
	printk("[HDMI]ep952_module_init begin\n");
	ret = disp_sys_script_get_item(key_name, "hdmi_used", &value, 1);
	if (1 == ret) {
		hdmi_used = value;
		if (hdmi_used) {
		#ifndef HDMI_POWER_GPIO_ENABLE
			for (i = 0; i < 4; i++) {
		...
		#else
        	ret = disp_sys_script_get_item(key_name, "hdmi_powen", (int *)powen_gpio,3);
        	if(ret == 3) {
            	powen_gpio_used = 1;			
