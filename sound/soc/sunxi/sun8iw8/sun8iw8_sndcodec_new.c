...
#define PA_CTRL_PIN_IS_PMU_GPIO
#ifdef PA_CTRL_PIN_IS_PMU_GPIO
...
static int codec_wrreg_prcm_bits(unsigned short reg, unsigned int mask, unsigned int value)
{
	...
	old	=	read_prcm_wvalue(reg,codec_analogadress);
	new	=	(old & ~mask) | value;
	write_prcm_wvalue(reg,new,codec_analogadress);
...
static int codec_init(struct sunxi_codec *sunxi_internal_codec)
{
    ...
    #ifndef PA_CTRL_PIN_IS_PMU_GPIO
    ...
    #else
        axp20x_gpio_set_value(PMU_GPIOX,0);
    #endif
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (sunxi_internal_codec->dac_enable == 0) {
            ...
            if(!sunxi_internal_codec->version_v3_used)
            {
                snd_soc_update_bits(codec,SUNXI_DAC_FIFOC ,(0x1<<LAST_SE), (0x0<<LAST_SE));//add by zhb
                snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<DACALEN, 0x1<<DACALEN);
    	        snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<DACAREN, 0x1<<DACAREN);
            }
	...
	case SND_SOC_DAPM_POST_PMD:
		if (sunxi_internal_codec->dac_enable > 0) {
            ...
            if(!sunxi_internal_codec->version_v3_used)
            {
                snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<DACALEN, 0x0<<DACALEN);
	            snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<DACAREN, 0x0<<DACAREN);
            }
				...
				int event)
{
	...
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
            ...
            {
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<LHPIS, 0x1<<LHPIS);
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<RHPIS, 0x0<<RHPIS);
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<LMIXEN, 0x1<<LMIXEN);
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<RMIXEN, 0x0<<RMIXEN);
    			snd_soc_update_bits(codec,ROMIXSC, 0x7f<<RMIXMUTE, 0x0<<RMIXMUTE);
    			snd_soc_update_bits(codec,LOMIXSC, 0x7f<<LMIXMUTE, 0x3<<LMIXMUTE);
    			snd_soc_update_bits(codec,HP_CTRL, 0x1<<LTRNMUTE, 0x1<<LTRNMUTE);
    			snd_soc_update_bits(codec,HP_CTRL, 0x1<<RTLNMUTE, 0x0<<RTLNMUTE);
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<LHPPAMUTE, 0x1<<LHPPAMUTE);
    			snd_soc_update_bits(codec,DAC_PA_SRC, 0x1<<RHPPAMUTE, 0x1<<RHPPAMUTE);
                /*set HPVOL volume*/
			    snd_soc_update_bits(codec,HP_VOLC, 0x3f<<HPVOL, sunxi_internal_codec->gain_config.speakervol);
            ...
            #else
                axp20x_gpio_set_value(PMU_GPIOX,1);
            #endif
			break;
		case SND_SOC_DAPM_PRE_PMD :
...
static int  sunxi_internal_codec_probe(struct platform_device *pdev)
{
    ...
    ret = of_property_read_u32(node, "pa_single_vol", &temp_val);
	if (ret < 0) {
		pr_err("[audio-codec]pa_single_vol configurations missing or invalid.\n");
		ret = -EINVAL;
		goto err1;
	} else {
     ...
     ret = of_property_read_u32(node, "headphone_direct_used", &temp_val);
	if (ret < 0) {
		pr_err("[audio-codec]headphone_direct_used configurations missing or invalid.\n");
		ret = -EINVAL;
		goto err1;
	} else {
		sunxi_internal_codec->headphone_direct_used = temp_val;
	}

     ret = of_property_read_u32(node, "version_v3_used", &temp_val);
	if (ret < 0) {
		pr_err("[audio-codec]version_v3_used configurations missing or invalid.\n");
		ret = -EINVAL;
		goto err1;
	} else {
		sunxi_internal_codec->version_v3_used = temp_val;
	}
